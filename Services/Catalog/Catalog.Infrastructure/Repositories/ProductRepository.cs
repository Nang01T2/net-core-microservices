...
namespace Catalog.Infrastructure.Repositories
{
    public class ProductRepository : IProductRepository, IBrandRepository, ITypesRepository
    {
        ...
        {
            var sorter = Builders<Product>.Sort;
            var sortDefn = sorter.Ascending("Name"); // Default
            if (!string.IsNullOrEmpty(catalogSpecParams.Sort))
            {
                switch (catalogSpecParams.Sort)
                {
                    case "priceAsc":
                        sortDefn = sorter.Ascending(p => p.Price);
                        break;
                    case "priceDesc":
                        sortDefn = sorter.Descending(p => p.Price);
                        break;
                    default:
                        sortDefn = sorter.Ascending(p => p.Name);
                        break;
